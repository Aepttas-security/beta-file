...
def ensure_user(db: Session):
    try:
        ...
        if not user:
            db.execute(text("INSERT INTO apt.apt_users_b (user_id, user_uuid, username, email, password_hash, created_by, created_date, last_updated_by, last_updated_date, last_dml_by, last_dml_date, last_ddl_by, last_ddl_date, program_id) VALUES (1, :u, 'admin', 'admin@shield.com', 'none', 'SYSTEM', now(), 'SYSTEM', now(), 'SYSTEM', now(), 'SYSTEM', now(), 1)"), {"u": str(uuid.uuid4())})
            db.commit()
    except: db.rollback()
...
def get_blocked(db: Session = Depends(get_db)):
    try:
        res = db.execute(text("SELECT b.phone_number, COALESCE(c.caller_name, 'Unknown'), b.reason, b.blocked_date FROM apt.apt_blocked_numbers_b b LEFT JOIN apt.apt_callers_b c ON RIGHT(b.phone_number, 10) = RIGHT(c.phone_number, 10) ORDER BY b.blocked_date DESC")).fetchall()
...
def add_block(req: BlockNumberRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if exists: db.execute(text("UPDATE apt.apt_blocked_numbers_b SET reason = :reason, last_dml_date = :dt WHERE phone_number = :num"), bp)
        else: db.execute(text("INSERT INTO apt.apt_blocked_numbers_b (blocked_number_uuid, user_id, caller_id, phone_number, reason, created_by, created_date, last_updated_by, last_updated_date, last_dml_by, last_dml_date, last_ddl_by, last_ddl_date) VALUES (:uuid, 1, :cid, :num, :reason, :by, :dt, :by, :dt, :by, :dt, :by, :dt)"), bp)
